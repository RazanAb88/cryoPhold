...
parser = argparse.ArgumentParser(description="Run CryoPhold pipeline on AlphaFold and cryo-EM data.")
parser.add_argument(
    "--path",
    type=str,
    required=True,
    help="Path to the directory containing the AlphaFold output (e.g. AF-output/)."
)
parser.add_argument(
    "--threshold",
    type=float,
    default=2.95,
    help="Threshold for the cryo-EM map (default: 2.95)."
)
parser.add_argument(
    "--resolution",
    type=float,
    default=3.05,
    help="Resolution of the cryo-EM map (default: 3.05).")
parser.add_argument(
    "--mask",
    type=str,
    default="sim",
    choices=["exp", "sim"],
    help="Masking option: 'exp' for experimental map, 'sim' for simulated map (default: 'sim')."
)
args = parser.parse_args()

if not os.path.exists(f"./iter"):
    os.makedirs(f"./iter")      
...
if (mask_arg == "exp"):
    ...
    exp_em_mask = exp_map_norm[mask_exp]
